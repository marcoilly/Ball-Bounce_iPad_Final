...
Shader "Custom/Gradienr_Glow_Color" {
	Properties {
...
//	   	 return float4(col.r, col.g, col.b, newOpacity);
 		...
 		float3 rimLighting =  _RimColor.xyz * pow(rim ,_RimPower);
...
//	      output.normalDir = normalize(mul(float4(input.normal,0.0f),_World2Object).xyz);
//	      output.tex = input.texcoord;
//	      output.color=input.color;
//	      return output;
//     }
